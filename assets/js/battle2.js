...
let enemyHP = [150, 160, 170, 180, 190, 200][Math.floor(Math.random() * 6)];
let dragStart = null;
...
async function shoot(angle, power, dmg) {
    ...
    const interval = setInterval(async () => {
        ...
        if (dist < 20) {
            ...
            if (enemyHP <= 0) {
                status.innerText = "你贏了！敵人被擊敗！獲得 30 金幣！";
                await updateCoins(30);
...
async function fireSkillBullet() {
    ...
    const interval = setInterval(async () => {
        ...
        if (dist < 20) {
            ...
            enemyHPBar.style.width = Math.max(0, enemyHP) + "%";
            if (enemyHP <= 0) {
                status.innerText = "你贏了！敵人被技能擊敗！獲得 30 金幣！";
                await updateCoins(30);
                setTimeout(() => {window.location.href = "lobby.html";}, 3000);
                return;
            }else{
...
async function enemyShoot() {
    ...
    const interval = setInterval(async () => {
        ...
        const dy = py - 20;
        const dist = Math.sqrt(dx * dx + dy * dy);
        const damage = [30, 35, 40][Math.floor(Math.random() * 3)];
